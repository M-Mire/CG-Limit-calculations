...
def find_most_limiting_envelope(env1: List[CGPoint], env2: List[CGPoint], 
    ...
    for point_info in all_points:
        ...
        if not is_intersection and env_num != current_env_num:
            continue
        print("limiting_envelope " ,limiting_envelope)
        ...
        if is_intersection:
            ...
            # CRITICAL FIX: For forward limits only, after intersection check both envelopes 
            # for points at same weight and choose the most limiting one
            if is_forward_limit:
                candidates_at_weight = []
                
                # Check envelope 1 for points at this weight
                for candidate_pt in clean_env1:
                    if candidate_pt.weight == weight and candidate_pt != pt:
                        candidates_at_weight.append(candidate_pt)
                
                # Check envelope 2 for points at this weight  
                for candidate_pt in clean_env2:
                    if candidate_pt.weight == weight and candidate_pt != pt:
                        candidates_at_weight.append(candidate_pt)
                
                # Add the most limiting candidate at this weight (highest MAC for forward)
                if candidates_at_weight:
                    print(candidates_at_weight)
                    best_candidate = max(candidates_at_weight, key=lambda p: p.mac)
                    if not limiting_envelope or best_candidate != limiting_envelope[-1]:
                        limiting_envelope.append(best_candidate)
            else:
                candidates_at_weight = []
                ...
                # Add the most limiting candidate at this weight (highest MAC for forward)
                if candidates_at_weight:
                    best_candidate = min(candidates_at_weight, key=lambda p: p.mac)
                    if not limiting_envelope or best_candidate != limiting_envelope[-1]:
                        limiting_envelope.append(best_candidate)
            
        else:
            # Regular envelope point from the current active envelope
            if env_num == current_env_num and len(limiting_envelope) == 0 or (not is_forward_limit and current_weight < weight) or (is_forward_limit and current_weight > weight):
                print("env_num !!!!!!!!!!= " , env_num, current_weight, weight)
                # Add point if it's not a duplicate of the previous point
                if not limiting_envelope or pt != limiting_envelope[-1]:
                    print(f"adding pt ={pt}, to limit_env")
                    limiting_envelope.append(pt)
                    current_weight = weight
